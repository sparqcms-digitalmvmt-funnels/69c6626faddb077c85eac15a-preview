...
const CHECKOUT_NEXT_PAGE_SLUG = "off/uuid20/ty";
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "DgJFumL6mzkKmBBdqsHZiseniVY9PjC9rHjtzQ1OP7fnOiiPR_A3mb4SIEMEPabl",
    action: "process",
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (typeof KOUNT_MERCHANT_ID !== 'undefined' && KOUNT_MERCHANT_ID) {
    let websiteSessionIdBase = '';
    if (typeof MVMT !== 'undefined' && typeof MVMT.getSessionId === 'function') {
      const mvmtSid = MVMT.getSessionId();
      if (mvmtSid) {
        websiteSessionIdBase = String(mvmtSid);
      }
    }

    if (!websiteSessionIdBase) {
      websiteSessionIdBase = new Date().getTime().toString(36) + Math.random().toString(36).substring(2, 12);
    }

    const merchantIdPrefix = KOUNT_MERCHANT_ID;
    const maxSessionPartLength = 32 - merchantIdPrefix.length;

    if (websiteSessionIdBase.length > maxSessionPartLength && maxSessionPartLength > 0) {
      websiteSessionIdBase = websiteSessionIdBase.substring(0, maxSessionPartLength);
    } else if (maxSessionPartLength <= 0) {
      console.warn("Kount Merchant ID is too long to form a valid Kount session ID.");
      websiteSessionIdBase = '';
    }
    kountSessionIdForSticky = merchantIdPrefix + websiteSessionIdBase;

    if (kountSessionIdForSticky.length > 32) {
      kountSessionIdForSticky = kountSessionIdForSticky.substring(0, 32);
    }

    localStorage.setItem('kount_session_id', kountSessionIdForSticky);

    const _kountOrigin = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') ? '//' : 'https://';
    const kountDataCollectorScript = document.createElement('script');
    kountDataCollectorScript.type = 'text/javascript';
    kountDataCollectorScript.async = true;
    kountDataCollectorScript.src = `${_kountOrigin}h.online-metrix.net/fp/tags.js?org_id=${KOUNT_ORG_ID}&session_id=${kountSessionIdForSticky}`;
    document.head.appendChild(kountDataCollectorScript);

    const kountNoscript = document.createElement('noscript');
    const kountIframe = document.createElement('iframe');
    kountIframe.src = `${_kountOrigin}h.online-metrix.net/fp/tags?org_id=${KOUNT_ORG_ID}&session_id=${kountSessionIdForSticky}`;
    kountIframe.style.cssText = 'width:100px; height:100px; border:0; position:absolute; top:-5000px; left:-5000px;';
    kountIframe.setAttribute('aria-hidden', 'true');
    kountIframe.setAttribute('title', 'Kount Data Collector');
    kountNoscript.appendChild(kountIframe);
    (document.body || document.documentElement).appendChild(kountNoscript);

    if (isTest && window.location.hostname === "localhost") {
      console.log('Kount SDK injected. Merchant ID:', KOUNT_MERCHANT_ID, 'Session ID for Kount:', kountSessionIdForSticky);
    }
  }
...
if (typeof validateAndSendToKlaviyo === "function") {
  try {
    ...
    var pageReadyPayload = {
      ...
      runId: "initial",
      hypothesisId: "KlaviyoLifecycle",
      data: { pageName: "checkout page", pageType: "Checkout" },
    };
